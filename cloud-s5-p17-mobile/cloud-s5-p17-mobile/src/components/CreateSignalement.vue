...
async function handleSubmit() {
  ...
  try {
    // Collecter les blobs des photos pour encodage en pixels
    const photoBlobs = photos.value.map((p) => p.blob);

    // 1. Créer le signalement dans Firebase avec les photos encodées en pixels
    const created = await signalementService.create({
      ...
      photoBlobs: photoBlobs.length > 0 ? photoBlobs : undefined,
    });

    const toast = await toastController.create({
      message: `Signalement créé avec ${photoBlobs.length} photo(s) !`,
      duration: 2000,
      color: 'success',
    });
    await toast.present();
